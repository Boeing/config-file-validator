...
func TestTraceGrouping(t *testing.T) {
	// The output after first format: "0 = 0\n\n\n#0\n"
	input := "0 = 0\n\n\n#0\n"
	tokens := NewLexer([]byte(input)).Tokenize()
	groups := NewGrouper(tokens).Group()

	for i, g := range groups {
		t.Logf("group[%d] kind=%d tokens=%d", i, g.Kind, len(g.Tokens))
		for j, tok := range g.Tokens {
			t.Logf("  tok[%d] kind=%d raw=%q", j, tok.Kind, tok.Raw)
		}
	}
}

func TestTraceFirstPass(t *testing.T) {
	input := "0=0\n \n \n #0"
	tokens := NewLexer([]byte(input)).Tokenize()
	groups := NewGrouper(tokens).Group()
	...
	out := NewPrinter(opts).Print(groups)
	t.Logf("Output: %q", out)
}

func TestTraceBlankBug(t *testing.T) {
	input := "0=0\n \n \n #0"
	tokens := NewLexer([]byte(input)).Tokenize()
	groups := NewGrouper(tokens).Group()

	t.Logf("Groups from input %q:", input)
	for i, g := range groups {
		nlCount := 0
		...
		t.Logf("  [%d] kind=%d nlCount=%d tokens=%d", i, g.Kind, nlCount, len(g.Tokens))
	}

	opts := DefaultPrintOptions()
	out := string(NewPrinter(opts).Print(groups))
	t.Logf("Output: %q", out)

	// Second pass
	tokens2 := NewLexer([]byte(out)).Tokenize()
	groups2 := NewGrouper(tokens2).Group()
	t.Logf("Groups from output %q:", out)
	...
	out2 := string(NewPrinter(opts).Print(groups2))
	t.Logf("Output2: %q", out2)
}
