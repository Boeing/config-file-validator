...
// MatchFile applies the finder filters to a single file path and returns it as
// a FileMetadata slice if it passes all filters, or an empty slice if it is
// excluded. Used by watch mode to check a single changed file without a full
// directory walk.
