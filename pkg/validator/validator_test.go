...
// Test_XMLRecursiveEntityDoesNotHang ensures that recursive DTD entity expansion
// doesn't cause the parser to hang indefinitely. See PR #509.
func Test_XMLRecursiveEntityDoesNotHang(t *testing.T) {
	t.Parallel()

	// Minimal recursive entity: a references b, b references a
...
<foo>&a;</foo>`)
	...
	require.False(t, valid, "recursive entity should be invalid")
	require.Error(t, err, "recursive entity should return an error")
	// The error should be a timeout/deadline exceeded error
